...
def _convert_to_sparse_format(x):
    ...
    print('new sparse format')
    for row, x_i in enumerate(x):
        for col, x_ij in enumerate(x_i):
            sparse['indices'].append((row, col))  ##col应该修订为 x_ij，
            sparse['values'].append(x_ij)         ##x_ij应该修订为 1
            #sparse['indices'].append((row, x_ij))  ##col应该修订为 x_ij，###实际实验显示，效果变差。####自己理解错误
            #sparse['values'].append(1)         ##x_ij应该修订为 1
