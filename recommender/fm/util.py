...
class FieldHandler(object):
    def __init__(self, train_file_path=None, test_file_path=None, label='Rating', category_columns=[], continuation_columns=[]):
        """
        ...
        self.read_data()
        #self.feature_nums = len(self.df.columns)
    ...
    def build_filed_dict(self):
        for column in self.df.columns:
            if column in self.category_columns:
                ...
                self.feature_nums += 1
        print("feature_nums:",self.feature_nums)
    ...
    def read_data(self):
        if self.train_file_path and self.test_file_path:
            ...
            self.df = pd.read_csv(self.train_file_path, index_col=False)
...
def transformation_data(file_path:str, field_hander:FieldHandler, label=None):
    ...
    """
    df_v = pd.read_csv(file_path)  #直接使用field_hander的df即可，不需要再次读取呀。
    if label:
    ...
    print("raise wrong",np.where(df_i > 5824))
    print("raise wrong",np.where(df_i < -2))
