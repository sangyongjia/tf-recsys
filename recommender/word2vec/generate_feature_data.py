...
import json
data = pd.read_csv("../../dataset/taobao_data/data_train.csv", dtype=str,names=["userid","item","category","buy_flag"] )
...
data_test = pd.read_csv("../../dataset/taobao_data/data_test.csv", dtype=str,names=["userid","item","category","buy_flag"])
data4 = pd.merge(data_test,word2id_df,left_on=["item"], right_on=["index"],how="left").fillna(-1)
data5 = pd.merge(data4,train_data,left_on=["encoderid"],right_on=["encoderid"],how="left").fillna(0)
